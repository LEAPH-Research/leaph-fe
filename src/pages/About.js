...
const TeamCard = ({ img, name, role, linkedin, website, description }) => (
  <div className="px-5 py-3 text-center items-center flex flex-col rounded-lg transition-all duration-300 hover:scale-105 bg-white text-green-900 h-96 w-96 hover:shadow-2xl">
    <img
      src={require(`../assets/team/${img}`)}
      alt={name}
      className="h-32 w-32 rounded-full"
    />
    <h3 className="cursor-default text-2xl mt-2 mb-1 font-josefin font-extrabold">
      {name}
    </h3>
    <p className="cursor-default text-xl font-semibold text-justify opacity-90 italic font-josefin">
      {role}
    </p>
    <div className="flex gap-3 mt-2">
      ...
      {website && (
        <a href={website} target="_blank" rel="noopener noreferrer">
          <img
            src={require("../assets/icons/web.png")}
            className="h-7 cursor-pointer"
            alt="Website"
          />
        </a>
    ...
    <ul className="list-disc text-start pl-2 mt-1 font-comfortaa cursor-default font-bold text-[15px]">
...
const ATeamCard = ({ img, name, role, linkedin, website, description }) => (
  <div className="px-5 py-3 text-center items-center flex flex-col rounded-lg transition-all duration-300 hover:scale-105 bg-white text-green-900 h-80 w-80 hover:shadow-2xl">
    ...
    <p className="cursor-default text-lg font-semibold text-center opacity-90 italic font-josefin">
      {role}
    </p>
    <div className="flex gap-3 mt-2">
      {linkedin && (
    ...
    <ul className="list-disc text-start w-11/12 mt-1 cursor-default font-comfortaa font-extrabold">
      {description.map((point, index) => (
        <li key={index}>{point}</li>
      ))}
    </ul>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-100 to-green-200">
      <header className="container mx-auto px-4 py-8 text-center">
        ...
        <h1 className="cursor-default text-5xl text-green-900 mb-4 font-josefin font-extrabold">
          About Us
        </h1>
      ...
      <main className="flex flex-col items-center justify-center pb-24">
        <h1 className="cursor-default text-4xl font-extrabold text-green-900 mb-4 font-josefin">
          Alumni
        </h1>
